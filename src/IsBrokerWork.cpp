/**
 * @file IsBrokerWork.cpp
 * @brief Is and Broker work分发服务器端接收发送处理
 * @author Yuantu
 * @version ver 1.0
 * @date 2012-11-30
 */
...
IsBrokerWork::~IsBrokerWork()
{
	pthread_mutex_destroy(&m_MainCri);
}

void IsBrokerWork::work(const RecvData &recv)
{
 ...
 */
void IsBrokerWork::BuildResponsePack(string& msg, _DFIELD_* pData)
{
	string result;
 ...
 */
void IsBrokerWork::WorkTimeout(const RecvData &recv)
{
...
int IsBrokerWork::BuildMd5(const char* szFile, string& strmd5, char* pBuf)
{
...
void IsBrokerWork::OpenAFile(const RecvData &recv)
{
	const char *psBuf = (recv.buffer).c_str();
...
void IsBrokerWork::CreateAList(const RecvData &recv)
{
	const char *psBuf = (recv.buffer).c_str();
...
void IsBrokerWork::WriteToFile(const RecvData &recv)
{
	const char *psBuf = (recv.buffer).c_str();
...
void IsBrokerWork::CloseAFile(const RecvData &recv)
{
	const char *psBuf = (recv.buffer).c_str();
...
void IsBrokerWork::CloseAList(const RecvData &recv)
{
	const char *psBuf = (recv.buffer).c_str();
	...
	if(m_mapList.find(strListName) != m_mapList.end())
	{
		...
		sprintf(szSymbol, "%s.%d%02d%02d", tmpData.m_Listname, stNow.tm_year + 1900, stNow.tm_mon + 1, stNow.tm_mday);
...
void IsBrokerWork::RemoveFile(const RecvData &recv)
{
	const char *psBuf = (recv.buffer).c_str();
 ...
 *
 */
void IsBrokerWork::CreateAFile(const RecvData &recv)
{
	const char *psBuf = (recv.buffer).c_str();
 ...
 *
 */
void IsBrokerWork::CheckAFile(const RecvData &recv)
{
	const char *psBuf = (recv.buffer).c_str();
 ...
 *
 */
void IsBrokerWork::RenameFile(const RecvData &recv)
{
	const char *psBuf = (recv.buffer).c_str();
 ...
 *
 */
void IsBrokerWork::RecvSingleFile(const RecvData &recv)
{
	const char *psBuf = (recv.buffer).c_str();
 ...
 *
 */
void IsBrokerWork::CheckMD5(const RecvData &recv)
{
	const char *psBuf = (recv.buffer).c_str();
 ...
 *
 */
void IsBrokerWork::GetFileMD5(const RecvData &recv)
{
	const char *psBuf = (recv.buffer).c_str();
 ...
 *
 */
void IsBrokerWork::FileChmod(const RecvData &recv)
{
	const char *psBuf = (recv.buffer).c_str();
	...
	tmpData.m_Result = 0;
	if(tmpData.m_OperateType != DF_FILE_CHMOD)
	{
 ...
 *
 */
void IsBrokerWork::KillProcess(const RecvData &recv)
{
	const char *psBuf = (recv.buffer).c_str();
 ...
 *
 */
void IsBrokerWork::ExecProcess(const RecvData &recv)
{
	const char *psBuf = (recv.buffer).c_str();
 ...
 *
 */
void IsBrokerWork::RPCShell(const RecvData &recv)
{
	const char *psBuf = (recv.buffer).c_str();
